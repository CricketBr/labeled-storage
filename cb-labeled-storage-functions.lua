...
local function make_one_tiny_chest(c, tech)
    ...
    for i = 1, 3 do
        ...
        local new_layer_info =
        {
            ...
            { file = 'a-square.png',               tint = cbimage.tints[c.letter], scale = 1,    entityshift = 'left-bot', iconshift = { -6, 6 } },
            { file = 'a-square.png',               tint = cbimage.tints.ltgrey,    scale = .75,  entityshift = 'left-bot', iconshift = { -6, 6 } },

            { file = c.letter .. '-left.png',      tint = cbimage.tints.dkgrey,    scale = 0.75, entityshift = 'left-bot', iconshift = { -6, 6 } },
            { file = 'smsquare-' .. i .. 'rt.png', tint = cbimage.tints.dkgrey,    scale = 0.75, entityshift = 'left-bot', iconshift = { -6, 6 } },




            -- { file = 'a-square.png',             tint = cbimage.tints.ltgrey,    scale = 1,    entityshift = 'left-top', iconshift = { -6, -6 } },
            -- { file = c.letter .. '.png',         tint = cbimage.tints.dkgrey,    scale = 0.75, entityshift = 'left-top', iconshift = { -6, -6 } },
            -- { file = 'frame08.png',              tint = cbimage.tints[c.letter], scale = 1,    entityshift = 'left-top', iconshift = { -6, -6 } },
            -- { file = 'smsquare-' .. i .. 'lt.png', tint = cbimage.tints.dkgrey,    scale = 1,    entityshift = 'left-top', iconshift = { -6, -6 } },
        }

        local images = cbimage.make_layers(lentity, new_layer_info)
        ...
        cbimage.add_icon_layers(litem, images.icons)
        data:extend({ litem })
        ...
        cbimage.add_icon_layers(lentity, images.icons)
        cbimage.add_animation_layers_to_logcont(lentity, images.animation_layers)
...
lsfns.tiny_chests = function(list, tech)
...
lsfns.logistic_labels = function()
    ...
    for _, pt in pairs(data.raw['logistic-container']) do
        ...
        if not (string.sub(pt.name, 1, 5) == 'cb-ls') then
            ...
            local letter = cbimage.letterlist[pt.logistic_mode]

            local new_layer_info =
            {
                { file = 'a-square.png',   tint = cbimage.tints[letter], scale = 1,    entityshift = 'left-bot', iconshift = { -6, 6 } },
                { file = 'a-square.png',   tint = cbimage.tints.ltgrey,  scale = 0.75, entityshift = 'left-bot', iconshift = { -6, 6 } },
                { file = letter .. '.png', tint = cbimage.tints.dkgrey,  scale = 0.75, entityshift = 'left-bot', iconshift = { -6, 6 } },
            }

            local images = cbimage.make_layers(pt, new_layer_info)
            cbimage.add_icon_layers(pt, images.icons) -- pt is the entity
            cbimage.add_icon_layers(data.raw.item[pt.name], images.icons)
            if pt.animation then
                cbimage.add_animation_layers_to_logcont(pt, images.animation_layers)
            elseif pt.picture then
                cbimage.add_picture_layers_to_container(pt, images.picture_layers)
            end
        end
    end
end
...
local function make_one_labst(args)
    ...
    L.new_layer_info =
    {
        { file = 'a-square.png',     tint = cbimage.tints.ltgrey, scale = 1,   iconshift = { -6, 6 }, entityshift = 'left-bot' },
        { file = L.corner,           tint = cbimage.tints.dkgrey, scale = 1,   iconshift = { -6, 6 }, entityshift = 'left-bot' },
        { file = 'frame08.png',      tint = L.colour,             scale = 1,   iconshift = { -6, 6 }, entityshift = 'left-bot' },
        { file = args.num .. '.png', tint = cbimage.tints.dkgrey, scale = .75, iconshift = { -6, 6 }, entityshift = 'left-bot' }
    }
    L.images = cbimage.make_layers(args.orig_entity, L.new_layer_info)
    ...
    if args.letter == 'EM' and args.num == '0' then
        ...
        L.item.name = L.name
        cbimage.add_icon_layers(L.item, L.images.icons)
    ...
    cbimage.add_icon_layers(L.entity, L.images.icons)

    if L.entity.type == 'container' then
        cbimage.add_picture_layers_to_container(L.entity, L.images.picture_layers)
    end

    if L.entity.type == 'storage-tank' then
        cbimage.add_picture_layers_to_storage_tank(L.entity, L.images.spriteNway_layers)
    end
...
lsfns.make_all_labst = function(list)
    local L = {}
    for _, v in pairs(list) do
        -- local v = list[1]


        local origpt = table.deepcopy(data.raw[v.type][v.name]) or nil
        if not origpt or not data.raw.technology[v.tech] then
            break
        end
